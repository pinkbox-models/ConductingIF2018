function v = axonIFextra(Iinj,Rc,dt,Nc,Dc,Lc,Ic)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Axonal spike conduction simulated with bounded integrate-and-fire (bEIF) 
% model stimulated by extracellular current
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input 
%  Iinj : extracellularly injected current at each time step [uA] (1D-vector)
%  Rc   : distance between the external current source and each node [mm]
%  dt   : time step [ms] 
...
rhoE = 3.0; % [Ohm.m] extracellular resistivity
trfE = rhoE ./ ( 4 * pi * Rc ); % [kOhm] transfer function between node and stimulus
Ve = zeros(Nc,length(Iinj)); 
for j=1:length(Iinj)-1
  Ve(:,j) = trfE * Iinj(j); % [kOhm]*[uA] = [mV]
end
...
iE = zeros(Nc, length(Iinj)); % axonal current due to extracellular stimulation

% initial values
v(:,1) = EL;  % initial membrane potential

%% diffusion matrix
[Adiff, Bdiff, gaxon] = diffusionmatrix(dt, Rax, Nc, Dcomp, Lcomp, Icomp, Cm);
...
for j=1:length(Iinj)-1
    ...
    % axonal current due to extracellular current injection: [mS]*[mV]=[uA]
    iE(:,j) = [gaxon;0] .* ( [ Ve(2:end,j); Ve(end,j) ] - Ve(:,j) ) ...
            + [0;gaxon] .* ( [ Ve(1,j); Ve(1:end-1,j) ] - Ve(:,j) );

    % derivatives I[uA] / C[uF] * dt[ms] = dv[mV]    
    dv_dt = ( iL(:,j) + iD(:,j) + iR(:,j) + iE(:,j) ) ./ Cm;  
