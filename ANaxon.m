function v = ANaxon(Iinj,dt,Nc,F)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Auditory nerve axonal spike conduction model 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input 
%  Iinj : injected current to each node at each time point [pA] (2D-vector)
%  dt   : time step [ms] 
...
Dc = 2.5; % [um] axonal diameter
Lc = 2.0; % [um] nodal length 

% frequency-dependent parameter
if F==0 % low freq
 Ic = 350; %[um] internodal length 
elseif F==1 % high freq
 Ic = 450; %[um] internodal length 
end
    ...
    for n=1:Nc
      if(spReady(n)==1) % if ready to spike, then check for spike occurrence
        ...
        if(v(n,j+1)<=Vre)
          ...
          spStart(n) = 0; % not starting spiking, of course 
          spReady(n) = 0; % and not yet ready for next spike
        end
      end
    end % end of for loop
